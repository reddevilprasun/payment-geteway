            ...
            try {
                ...
                if (data.status && data.data) {
                    ...
                    setTodayStats({
                        ...
                        successRate: `100%`,
                        avgTransactionValue: `₹${(Number(dayWise.totalAmount.value) / 100 / Number(dayWise.totalCount)).toFixed(2)}`,
                    ...
                    // transactions
                    const transactions: Transaction[] = dayWise.orderList.map((order: ApiOrder) => ({
                        id: order.bizOrderId,
                        customer: order.additionalInfo.customerName || order.nickName || "Unknown",
                        amount: `₹${Number(order.payMoneyAmount.value) / 100}`,
                        status: order.orderStatus === "SUCCESS" ? "completed" : order.orderStatus === "PENDING" ? "pending" : "failed",
                    ...
                    const peakData = analyticsData.hourlyData.length
                        ? analyticsData.hourlyData.reduce((max, curr) => curr.amount > max.amount ? curr : max, analyticsData.hourlyData[0])
                        : {hour: "--", amount: 0};

                    const avgHour = analyticsData.hourlyData.length
                        ? (analyticsData.hourlyData.reduce((sum, curr) => sum + curr.amount, 0) / analyticsData.hourlyData.length).toFixed(2)
                        : "0";
                    ...
                    <div className="flex justify-between items-center h-16">
                        <div className="flex items-center space-x-8">
                            ...
                            <nav className="hidden md:flex items-center space-x-6">
                                ...
                                <Link href="/profile"
                                      className="text-gray-600 hover:text-purple-600 transition-colors">Profile</Link>
                                <Link href="/docs" className="text-gray-600 hover:text-purple-600 transition-colors">API
                                    Docs</Link>
                            </nav>
            ...
            <div className="max-w-7xl mx-auto px-2 sm:px-4 lg:px-8 py-4">
                        ...
                        <div className="relative">
                            ...
                            <div className="flex items-end justify-between h-64 px-4 pt-4">
                                {analyticsData.hourlyData.map((data, index) => {
                                    const height = (data.amount / Math.max(...analyticsData.hourlyData.map(d => d.amount))) * 100;
                                    const isHighest = data.amount === Math.max(...analyticsData.hourlyData.map(d => d.amount));
                                    return (
                                        <div key={index} className="flex flex-col items-center group cursor-pointer">
                                            {/* Tooltip */}
                                            <div
                                                className="opacity-0 group-hover:opacity-100 transition-opacity duration-200 mb-2 bg-gray-900 text-white text-xs rounded-lg px-3 py-2 whitespace-nowrap">
                                                <div className="font-semibold">₹{data.amount.toLocaleString()}</div>
                                                <div className="text-gray-300">{data.hour}</div>
                                                <div
                                                    className="absolute bottom-0 left-1/2 transform -translate-x-1/2 translate-y-1 w-2 h-2 bg-gray-900 rotate-45"></div>
                                            </div>

                                            {/* Bar */}
                                            <div
                                                className="relative w-8 bg-gray-100 rounded-t-lg overflow-hidden group-hover:shadow-lg transition-all duration-300">
                                                <div
                                                ...
                                                >
                                                    {/* Shine Effect */}
                                                    <div
                                                        className="absolute inset-0 bg-gradient-to-r from-transparent via-white to-transparent opacity-0 group-hover:opacity-20 transform -skew-x-12 transition-all duration-500"></div>
                                                </div>
                                                ...
                                                {isHighest && (
                                                    <div
                                                        ...
                                                        <div
                                                            className="w-2 h-2 bg-yellow-400 rounded-full animate-pulse"></div>
                                                    </div>
                                                )}
                                            </div>
                                            ...
                                            {/* Hour Label */}
                                            <div
                                                className="mt-3 text-xs text-gray-600 font-medium group-hover:text-purple-600 transition-colors">
                                                {data.hour}
                                            </div>
                                    ...
                                    {gatewayProfile ? (
                                        <div className="space-y-2">
                                            <div className="flex items-center justify-between">
                                                ...
                                                <span
                                                    className="font-medium text-gray-900">{gatewayProfile.username || gatewayProfile.firstName}</span>
                                            </div>
                                            <div className="flex items-center justify-between">
                                                <span className="text-sm text-gray-600">Mobile</span>
                                                <span
                                                    className="font-medium text-gray-900">{gatewayProfile.mobile}</span>
                                            </div>
                                            <div className="flex items-center justify-between">
                                                <span className="text-sm text-gray-600">Email</span>
                                                <span
                                                    className="font-medium text-gray-900">{gatewayProfile.email}</span>
                                            </div>
                                            <div className="flex items-center justify-between">
                                                <span className="text-sm text-gray-600">MID</span>
                                                <span
                                                    className="font-medium text-gray-900">{gatewayProfile.merchants?.[0]?.mid}</span>
                                            </div>
                                        </div>
                                    ) : (
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    <div className="flex items-center justify-between mb-2">
                                        ...
                                        <span
                                            className="text-sm text-gray-900">{currentPlan.usedToday} / {currentPlan.transactionLimit}</span>
                                    </div>
                                ...
                                <tr key={transaction.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="flex items-center">
                                            ...
                                            <div className="ml-3">
                                                <div
                                                    className="text-sm font-medium text-gray-900">{transaction.customer}</div>
                ...
                <div>
                    <div className="text-center mb-6">
                        <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-2">Upgrade Your Plan</h2>
                        <p className="text-gray-600 text-sm sm:text-base">Scale your business with our flexible pricing
                            options</p>
                    </div>
